...
def next_user(location):
    ...
    avail_machine = get_avail_machine_id(location=location)
    if next_user:
        return jsonify({"user_id": (next_user for next_user in LaundQueue.query.filter_by(location=location).first()),
         "machine_id":avail_machine})
